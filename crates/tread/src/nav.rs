...
impl Reader {
    ...
    pub fn nav_word_back(&mut self, big: bool) {
        ...
        if let Some(text) = self.visual_lines.get(cur).map(|vl| vl.text.as_str()) {
            ...
            if !bytes.is_empty() && self.cursor_x > 0 {
                ...
                x -= 1;
                while x > 0 && bytes[x].is_ascii_whitespace() {
                    x -= 1;
                }
                if !bytes[x].is_ascii_whitespace() {
                    let here = char_kind(bytes[x], big);
                    while x > 0 && char_kind(bytes[x - 1], big) == here {
                        x -= 1;
                    }
        ...
        if let Some(text) = self.visual_lines.get(cur).map(|vl| vl.text.as_str()) {
            ...
            if !bytes.is_empty() && self.cursor_x < bytes.len() {
                ...
                if at_end {
                    ...
                    let mut y = x + 1;
                    while y < bytes.len() && bytes[y].is_ascii_whitespace() {
                        y += 1;
                    }
                    if y < bytes.len() {
                        let kind = char_kind(bytes[y], big);
                        while y + 1 < bytes.len() && char_kind(bytes[y + 1], big) == kind {
                            y += 1;
                        }
    ...
    pub fn nav_sentence_forward(&mut self) {
        ...
        while line < total {
            let Some(text) = self.visual_lines.get(line).map(|vl| vl.text.as_str()) else {
                break;
            };
            let bytes = text.as_bytes();
            while col < bytes.len() {
    ...
    pub fn find_char_in_line(&self, ch: char, kind: FindKind) -> Option<usize> {
        ...
        match kind {
            ...
            FindKind::T => (x + 1..bytes.len())
                .find(|&i| bytes[i] == target)
