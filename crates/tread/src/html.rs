  ...
  let body = doc
    ...
    .find_map(|n| {
      ElementRef::wrap(n).and_then(|el| {
        if el.value().name() == "body" { Some(el) } else { None }
      })
    })
...
fn skipped_tags() -> &'static HashSet<&'static str> {
  ...
  TAGS.get_or_init(|| {
    [
      ...
      "title", // page title is metadata, not body content
    ]
    .iter()
    .copied()
    .collect()
...
impl Walker {
  ...
  fn handle_element(&mut self, el: ElementRef, name: &str) {
    ...
    match name {
      ...
      "p" | "div" | "section" | "article" | "main" | "header" | "footer" | "aside" | "nav" | "figure" => {
            ...
            c.value()
              ...
              .and_then(|cls| {
                cls.split_whitespace().find_map(|c| {
                  c.strip_prefix("language-").map(str::to_string)
                })
              })
          });
        let mut buf = String::new();
...
mod tests {
  ...
  fn bold_italic_code_become_inline_styles() {
    let blocks =
      parse("<p>This is <strong>bold</strong> and <em>italic</em> and <code>code</code>.</p>");
    let spans: Vec<&InlineSpan> = blocks.iter().find_map(|b| match b {
  ...
  fn link_carries_href_on_span() {
    ...
    assert!(spans.iter().any(|s| s.text == "click" && s.url.as_deref() == Some("https://example.com")));
  }
...
let x = 1;</code></pre>"#,
  ...
  fn script_and_style_are_dropped() {
    ...
    let combined: String = blocks
      .iter()
      .filter_map(|b| match b {
        Block::StyledLine(spans) => Some(spans.iter().map(|s| s.text.as_str()).collect::<String>()),
        _ => None,
      })
      .collect();
