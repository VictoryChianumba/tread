  ...
  fn start(&mut self, tag: Tag) {
    match tag {
      ...
      Tag::CodeBlock(kind) => {
        ...
        self.code_block = Some(CodeBlockState { lang, buf: String::new() });
      }
      Tag::List(start) => {
    ...
    match end {
      ...
      TagEnd::CodeBlock => {
        if let Some(cb) = self.code_block.take() {
          ...
          self.blocks.push(Block::CodeBlock { lang: cb.lang, lines });
          self.blocks.push(Block::Blank);
        }
      }
      TagEnd::List(_) => {
      ...
      TagEnd::Item => {
        ...
        self.blocks.push(Block::ListItem { depth, marker, content });
        self.in_item = false;
      }
