...
pub fn word(reader: &Reader, big: bool, around: bool) -> Option<String> {
  ...
  let is_word_byte = |b: u8| -> bool {
    if big { !b.is_ascii_whitespace() } else { b.is_ascii_alphanumeric() || b == b'_' }
  };
  ...
  while end + 1 < bytes.len() && is_word_byte(bytes[end + 1]) { end += 1; }
  ...
  while a_end < bytes.len() && bytes[a_end].is_ascii_whitespace() { a_end += 1; }
  let mut a_start = start;
  if a_end == inner_end_excl {
    // No trailing ws — eat leading.
    while a_start > 0 && bytes[a_start - 1].is_ascii_whitespace() { a_start -= 1; }
  }
...
fn find_quote_before(bytes: &[u8], target: u8, max_inclusive: usize) -> Option<usize> {
  ...
  (0..=cap).rev().find(|&i| bytes[i] == target && !preceded_by_backslash(bytes, i))
}
...
pub fn pair(reader: &Reader, open: char, close: char, around: bool) -> Option<String> {
  if !open.is_ascii() || !close.is_ascii() { return None; }
  let vl = reader.visual_lines.get(reader.current_line())?;
  let text = vl.text.as_str();
  let bytes = text.as_bytes();
  if bytes.is_empty() { return None; }
  ...
  for i in (0..=x).rev() {
    if bytes[i] == c { depth += 1; }
    else if bytes[i] == o {
      if depth == 0 { opener = Some(i); break; }
      depth -= 1;
    }
  }
  let opener = opener?;
  ...
  for i in (opener + 1)..bytes.len() {
    if bytes[i] == o { depth += 1; }
    else if bytes[i] == c {
      if depth == 0 { closer = Some(i); break; }
      depth -= 1;
    }
  }
  let closer = closer?;
...
pub fn sentence(reader: &Reader, around: bool) -> Option<String> {
  ...
  let mut start = (0usize, 0usize);
  {
    ...
    'outer: for line in 0..total {
      let Some(text) = reader.visual_lines.get(line).map(|vl| vl.text.as_str()) else { continue };
      let bytes = text.as_bytes();
  ...
  let mut found_end = false;
  {
    ...
    let mut col = start.1;
    while line < total {
      let Some(text) = reader.visual_lines.get(line).map(|vl| vl.text.as_str()) else { break };
      let bytes = text.as_bytes();
      while col < bytes.len() {
        let b = bytes[col];
        if matches!(b, b'.' | b'!' | b'?') {
  ...
  if !found_end {
    ...
    let last_len = reader.visual_lines.get(last).map(|vl| vl.text.len()).unwrap_or(0);
  ...
  loop {
    let Some(text) = reader.visual_lines.get(a_end.0).map(|vl| vl.text.as_str()) else { break };
    let bytes = text.as_bytes();
    if a_end.1 + 1 < bytes.len() && bytes[a_end.1 + 1].is_ascii_whitespace() {
      ...
      let Some(next_text) = reader.visual_lines.get(a_end.0).map(|vl| vl.text.as_str()) else { break };
...
fn join_range(reader: &Reader, start: (usize, usize), end: (usize, usize)) -> Option<String> {
  if start.0 > end.0 || (start.0 == end.0 && start.1 > end.1) { return None; }
  let mut out = String::new();
  for line in start.0..=end.0 {
    let text = reader.visual_lines.get(line).map(|vl| vl.text.as_str())?;
    let lo = if line == start.0 { start.1 } else { 0 };
    let hi_excl = if line == end.0 { end.1.saturating_add(1).min(text.len()) } else { text.len() };
    if line > start.0 { out.push('\n'); }
...
mod tests {
  ...
  fn reader_with(lines: &[&str]) -> Reader {
    let blocks: Vec<Block> = lines.iter().map(|l| {
      if l.is_empty() { Block::Blank } else { Block::Line(l.to_string()) }
    }).collect();
