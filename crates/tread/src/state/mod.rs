...
impl FigureEntry {
    ...
    ///
    /// `cell_w_px` / `cell_h_px` are passed through to
    /// `compute_cell_footprint` for aspect-correct sizing — values come
    /// from `doc_model::CELL_W_PX` / `CELL_H_PX`, kept out of the
    /// signature so this stays free of doc-model implementation
    /// constants.
            ...
            .map(|(hi, hrow)| {
                ...
                let cells: Vec<HeaderLabelPlacement> = hrow
                    ...
                    .map(|hcell| {
                        ...
                        let last_col = (col_cursor + span - 1).min(
                            col0_positions.len().saturating_sub(1),
                        );
