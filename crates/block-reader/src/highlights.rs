...
impl HighlightSet {
  ...
  pub fn add(&mut self, mut h: Highlight) {
    if h.byte_end <= h.byte_start { return; }
  ...
  pub fn remove_at(&mut self, block_idx: usize, byte: usize) -> bool {
    if let Some(idx) = self.highlights.iter().position(|h| {
      h.block_idx == block_idx && byte >= h.byte_start && byte < h.byte_end
    }) {
...
pub fn load(key: &str) -> HighlightSet {
  let Some(p) = path(key) else { return HighlightSet::default() };
  let Ok(data) = std::fs::read_to_string(&p) else { return HighlightSet::default() };
...
mod tests {
  ...
  fn h(block_idx: usize, byte_start: usize, byte_end: usize) -> Highlight {
    Highlight { block_idx, byte_start, byte_end }
  }
