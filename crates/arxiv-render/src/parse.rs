...
impl InlineBuilder {
    ...
    fn push_plain(&mut self, s: &str) {
        self.plain_buf.push_str(s);
    }
    fn push_char(&mut self, c: char) {
        self.plain_buf.push(c);
    }

    fn is_empty(&self) -> bool {
        self.plain_buf.is_empty() && self.spans.is_empty()
    }
    ...
    fn flush_plain(&mut self) {
        if !self.plain_buf.is_empty() {
            self.spans
                .push(InlineSpan::plain(std::mem::take(&mut self.plain_buf)));
        }
    }
...
const FULL_SKIP_ENVS: &[&str] = &["tikzpicture", "minipage", "pgfpicture"];
...
const CAPTION_ENVS: &[&str] = &["figure", "figure*", "wrapfigure", "subfigure"];
...
pub fn to_blocks(sources: Vec<(String, String)>) -> Vec<Block> {
    ...
    // Falls back to hand-parser if Pandoc is not installed or fails.
    if let Ok(blocks) = crate::pandoc_parse::try_pandoc(&sources) {
        return blocks;
    }
    ...
    while changed {
        ...
        for (begin_tag, end_tag) in &[
            ...
            if let Some(reordered) =
                try_move_table_before_section(&result, begin_tag, end_tag, SECTION_CMDS)
            {
                result = reordered;
                changed = true;
                break;
...
fn reorder_t_floats(body: &str) -> String {
    ...
    loop {
        let Some((t_start, t_end)) = next_t_float(&text, scan_from, VARIANTS) else {
            break;
        };
            ...
            match cmd.as_str() {
                ...
                "begin" => {
                    ...
                    if display_math_envs.iter().any(|e| *e == env.as_str()) {
                        ...
                        if !lines.is_empty() {
                            out.push(Block::DisplayMath { lines, num: eq_num });
                        }
                        continue;
                    }
                    ...
                    if matches!(env.as_str(), "quote" | "quotation" | "epigraph") {
                        ...
                        if !cleaned.trim().is_empty() {
                            out.push(Block::Quote(vec![InlineSpan::italic(
                ...
                "texttt" | "mathtt" | "textnormal" => {
                    let (content, skip) = read_braced_arg(&text, i);
                    i += skip;
                    builder.push_styled(content, |s| InlineSpan {
                        monospace: true,
                        ..s
                    });
                }
                ...
                "text" | "mathrm" | "mathcal" | "mathbb" | "overline" | "textsubscript"
                | "textsuperscript" | "textsf" | "textsc" | "textsl" | "textrm" | "textmd"
                | "textup" => {
                    let (content, skip) = read_braced_arg(&text, i);
                    i += skip;
                ...
                "verb" => {
                    if i < len {
                        ...
                        builder.push_styled(verbatim, |s| InlineSpan {
                            monospace: true,
                            ..s
                        });
                    }
        ...
        (None, Some((depth, marker))) => {
            ...
            if !trimmed.is_empty() {
                out.push(Block::ListItem {
                    depth,
                    marker,
                    content: vec![InlineSpan::plain(trimmed)],
                });
            }
        }
        (None, None) => {
...
fn parse_algorithmic_body(body: &str) -> Vec<String> {
    ...
    while i < len {
        ...
        match cmd.to_ascii_lowercase().as_str() {
            ...
            "algorithmic" | "algorithmicx" | "begin" | "end" | "algrenewcommand"
            | "algnewcommand" | "algblock" | "algrequire" | "algensure" | "algsetblock"
...
fn parse_multicolumn_cell(s: &str) -> Option<(String, usize)> {
    ...
    let (n_str, s1) = read_braced_arg(&chars, i);
    i += s1;
    let (_align, s2) = read_braced_arg(&chars, i);
    i += s2;
    ...
    while i < chars.len() {
        ...
        if chars[i] == '\\' && i + 1 < chars.len() {
            ...
            match cmd.as_str() {
                ...
                | "citenum" | "citeyearpar" | "shortcite" | "fullcite" | "autocite"
                | "parencite" | "textcite" | "footcite" => {
                ...
                "vspace" | "hspace" | "vspace*" | "hspace*" | "vskip" | "hskip" | "medskip"
                | "smallskip" | "bigskip" | "noindent" | "centering" | "raggedright"
                | "raggedleft" | "phantom" | "vphantom" | "hphantom" => {
                    if i < chars.len() && chars[i] == '{' {
                        let (_, skip) = read_braced_arg(&chars, i);
                        i += skip;
                    }
...
fn process_prose(text: &str, macros: &HashMap<String, (usize, String)>) -> Vec<String> {
    ...
    while i < chars.len() {
        ...
        if chars[i] == '\\' && i + 1 < chars.len() {
            ...
            i += 1 + consumed;
            match cmd.as_str() {
                "emph" | "textbf" | "textit" | "texttt" | "textsf" | "textsc" | "textsl"
                | "textrm" | "textmd" | "textup" | "textnormal" => {
                    let (content, skip) = read_braced_arg(&chars, i);
                    i += skip;
                    out.push_str(&content);
                }
