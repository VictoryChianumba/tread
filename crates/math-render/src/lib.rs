...
fn preprocess(src: &str) -> String {
    ...
    const MULTI_EQ_ENVS: &[&str] = &[
        "align", "aligned", "gather", "gathered", "eqnarray", "cases", "matrix", "pmatrix",
        "bmatrix", "vmatrix", "Vmatrix",
    ];
    let has_multi_eq = MULTI_EQ_ENVS
        .iter()
        .any(|e| s.contains(&format!("\\begin{{{e}}}")) || s.contains(&format!("\\begin{{{e}*}}")));
    if has_multi_eq {
    ...
    while i < len {
        ...
        if c == '\\' && i + 1 < len {
            ...
            match cmd.as_str() {
                ...
                "frac" | "tfrac" | "dfrac" => {
                    let (num, skip1) = read_braced(&chars, i);
                    i += skip1;
                    let (den, skip2) = read_braced(&chars, i);
                    i += skip2;
                ...
                "sqrt" => {
                    ...
                    let (inner, skip) = read_braced(&chars, i);
                    i += skip;
                    out.push_str(&format!("√{}", strip_latex(&inner)));
                }
                // Keep braced content.
                "mathbb" | "mathcal" | "mathbf" | "mathit" | "mathsf" | "mathrm" | "text"
                | "mbox" | "hbox" | "operatorname" | "boldsymbol" | "bm" | "tilde" | "hat"
                | "bar" | "vec" | "dot" | "ddot" | "widehat" | "widetilde" | "overline"
                | "underline" | "overbrace" | "underbrace" => {
                    let (content, skip) = read_braced(&chars, i);
                    i += skip;
                ...
                "label" | "tag" | "nonumber" | "notag" | "vspace" | "hspace" | "phantom"
                | "color" | "begin" | "end" => {
                    if i < len && chars[i] == '{' {
                        let (_, skip) = read_braced(&chars, i);
                        i += skip;
                    }
                }
