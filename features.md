...
- **Cross-reference jumping** — section/figure/table refs and citations render with the link colour and an underline. `Enter` on one jumps to the target (one line above so the target is fully visible); `Ctrl+O` rewinds. `Shift+Enter` shows a citation's bib entry in a popup without leaving your reading position. Phrases like "Table 3" and "Section 6.2" are styled as a whole, not just the number.
